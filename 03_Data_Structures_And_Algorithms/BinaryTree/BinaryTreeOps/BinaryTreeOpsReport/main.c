...
boolean contains(int id){ 
  User* user = gettUser(id);	// Search for user with given id
...
void fileInput(FILE* file){
  ...
  int i;					// iterator
  int addedUserCount = 0;			// For Reporting Purposes
  while( !feof(file)  ){			// While we haven't reach EOF
    ...
    addedUserCount++;
    if( addedUserCount % 10 == 0 ){
      printf("Added 10 More Users, Calling printInOrder: \n");
      printInOrder(root);
      printf("\n");
    }
  }
